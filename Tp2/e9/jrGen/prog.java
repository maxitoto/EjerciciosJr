...
return null;
    ...
    private static void filosofointTovoid(Op_ext.JRProxyOp retOp, Cap_ext fretOp, edu.ucdavis.jr.RemoteHandler handler, java.lang.Object [] JRargs)
    {
        try    {
            // GetMethod 1
            int id = ((Number) JRargs[0]).intValue();
            {
                try {
                    // Begin Expr2
                    Thread.sleep(2500);
                ...
                int mDerecha = id;
                int mizquierda = (id + 1 > N - 1 ? 0 : id + 1);
                if (id == 0) {
                    int aux = mizquierda;
                    // Begin Expr2
                    mizquierda = mDerecha;
                ...
                JRLoop1: for (int i = 0; i < iteraciones; i++) {
                    {
                        jrvm.sendAndDie();
                        Recv_ext recv_voidTovoid = palillos[mizquierda].recv();
                    ...
                    {
                        jrvm.sendAndDie();
                        Recv_ext recv_voidTovoid = palillos[mDerecha].recv();
                        jrvm.ariseAndReceive();
                        if (recv_voidTovoid.retOp != null)
                            recv_voidTovoid.retOp.send(jrvm.getTimestamp(), (java.lang.Object[]) null);
                    }
                    ...
                    palillos[mizquierda].send(jrvm.getTimestamp(), (edu.ucdavis.jr.RemoteHandler)null, (java.lang.Object [])null);
                    palillos[mDerecha].send(jrvm.getTimestamp(), (edu.ucdavis.jr.RemoteHandler)null, (java.lang.Object [])null);
                    // Begin Expr2
    ...
    static {
        try {
            for (int id = 0; id < N; id++) {
                op_filosofo_intTovoid.send(jrvm.getTimestamp(), new java.lang.Object [] {id});
            }
    ...
    protected void JRinit() {
    	...
    	jrresref = new JRprog(op_filosofo_intTovoid);
    	this.JRcalled = true;
    }
