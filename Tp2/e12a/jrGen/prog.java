...
public class prog extends java.lang.Object {
    ...
    static Op_ext.JRProxyOp JRget_op_entraRojo_voidTovoid()
    {
        return op_entraRojo_voidTovoid;
    }
    
    public static Op_ext.JRProxyOp op_entraRojo_voidTovoid;
    ;
    static    {
        try    {
            op_entraRojo_voidTovoid = new Op_ext_.JRProxyOp(new InOp_ext_impl());
            {
                long JRTotal = 1;
                if (JRTotal < 0) throw new jrRuntimeError("op initialized with a negative value");
                for (long JRiter = 0; JRiter < JRTotal; JRiter++)
                    op_entraRojo_voidTovoid.send(jrvm.getTimestamp(), (java.lang.Object []) null);
            }
        } catch (Exception e) { throw new jrRuntimeError(e.toString());}
    }
    
    static Op_ext.JRProxyOp JRget_op_entraBlanco_voidTovoid()
    {
        return op_entraBlanco_voidTovoid;
    }
    
    public static Op_ext.JRProxyOp op_entraBlanco_voidTovoid;
    ;
    static    {
        try    {
            op_entraBlanco_voidTovoid = new Op_ext_.JRProxyOp(new InOp_ext_impl());
            {
                long JRTotal = 2;
                if (JRTotal < 0) throw new jrRuntimeError("op initialized with a negative value");
                for (long JRiter = 0; JRiter < JRTotal; JRiter++)
                    op_entraBlanco_voidTovoid.send(jrvm.getTimestamp(), (java.lang.Object []) null);
            }
        } catch (Exception e) { throw new jrRuntimeError(e.toString());}
    }
...
return null;
    ...
    protected void JRinit() {
    	...
    	jrresref = new JRprog(op_x_voidTovoid, op_entraBlanco_voidTovoid, op_entraRojo_voidTovoid);
    	this.JRcalled = true;
    }
